...
class Control(object):
    def __init__(self):
        ...
        self.calibration_leg_point=self.readFromTxt(u'point')
        ...
        self.order=[u'',u'',u'',u'',u'',u'']
        self.calibration()
        self.setLegAngle()
        self.Thread_conditiona=threading.Thread(target=self.condition)
    def readFromTxt(self,filename):
        file1 = open(filename + u".txt", u"r")
        list_row = file1.readlines()
        list_source = []
        for i in xrange(len(list_row)):
            column_list = list_row[i].strip().split(u"\t")
            list_source.append(column_list)
        for i in xrange(len(list_source)):
            for j in xrange(len(list_source[i])):
                list_source[i][j] = int(list_source[i][j])
        file1.close()
        return list_source

    def saveToTxt(self,list, filename):
        file2 = open(filename + u'.txt', u'w')
        for i in xrange(len(list)):
            for j in xrange(len(list[i])):
                file2.write(unicode(list[i][j]))
                file2.write(u'\t')
            file2.write(u'\n')
        file2.close()
    ...
    def setLegAngle(self):
        if self.checkPoint():
            ...
            print u"This coordinate point is out of the active range"
    def checkPoint(self):
        flag=True
    ...
    def condition(self):
        while True:
            if self.flag==0x00:
                time.sleep(0.01)
            if (time.time()-self.timeout)>10 and  self.timeout!=0 and self.order[0]==u'':
            ...
            if cmd.CMD_POSITION in self.order and len(self.order)==4:
                ...
                if self.order[2] ==u"0" and self.order[3] ==u"0":
                    self.run(self.order)
                    self.order=[u'',u'',u'',u'',u'',u''] 
                else:
                    if self.flag!=0x03:
                ...
                if len(self.order) >=2:
                    if self.order[1]==u"one":
                        ...
                        self.calibration()
                        self.setLegAngle()
                    elif self.order[1]==u"save":
                        self.saveToTxt(self.calibration_leg_point,u'point')
                self.order=[u'',u'',u'',u'',u'',u''] 
    def relax(self,flag):
        if flag:
            self.servo.relax()
